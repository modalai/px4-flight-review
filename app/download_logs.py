#! /usr/bin/env python3
""" Script to download logs from flight-review.modalai.com

The server requires a modalai.com Google sign-in. Sign in once in a browser, copy the
value of the `_oauth2_proxy` cookie (DevTools > Application > Cookies) and pass it with
--cookie or the FLIGHT_REVIEW_COOKIE environment variable. The cookie is valid for 7 days.
"""
...
def get_arguments():
    ...
    parser = argparse.ArgumentParser(description='Python script for downloading logs '
                                                 'from flight-review.modalai.com. Requires a '
                                                 'modalai.com sign-in (see --cookie).',
    ...
    parser.add_argument('--db-info-api', type=str, default="https://flight-review.modalai.com/dbinfo",
                        help='The url at which the server provides the dbinfo API.')
    parser.add_argument('--download-api', type=str, default="https://flight-review.modalai.com/download",
    ...
    parser.add_argument('--cookie', type=str, default=os.environ.get('FLIGHT_REVIEW_COOKIE'),
                        help='Value of the _oauth2_proxy cookie from a browser signed in to '
                             'flight-review.modalai.com with a modalai.com account. '
                             'Defaults to the FLIGHT_REVIEW_COOKIE environment variable.')
...
def check_signed_in(response):
    """
    Exit with a clear message if the server redirected to the sign-in page
    """
    if response.is_redirect and '/oauth2/' in response.headers.get('Location', ''):
        print(f'\n{"="*60}')
        print('ERROR: The server requires a modalai.com sign-in.')
        print('Sign in at https://flight-review.modalai.com in a browser, copy the value')
        print('of the _oauth2_proxy cookie and pass it with --cookie (or set')
        print('FLIGHT_REVIEW_COOKIE). If you did, the cookie has probably expired.')
        print(f'{"="*60}\n')
        sys.exit(1)
...
def download_with_retry(url, entry_id, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            request = SESSION.get(url=url + "?log=" + entry_id, stream=True, timeout=10*60,
